...
const stepSchema = z.object({
  action: z
    .enum([
      "navigate", "click", "type", "screenshot", "get_content", "eval", "wait",
      "sleep", "scroll", "select", "press_key", "hover",
  ...
  selector: z
    .string()
    .optional()
    .describe("Element selector for click/type/wait/hover/extract_text/select/press_key actions"),
  text: z
    .string()
    .optional()
    .describe("Text for 'type' action, or visible text for 'click'/'wait' (when no selector)"),
  code: z
    .string()
    .optional()
    .describe("JavaScript code for 'eval' action"),
  timeout: z
    .number()
    .int()
    .min(0)
    .optional()
    .describe("Timeout in ms. For 'sleep': pause duration. For others: max wait time"),
  selectorType: z
  ...
  key: z
    .string()
    .optional()
    .describe("Key name for 'press_key' (e.g. 'Enter', 'Escape', 'Tab', 'ArrowDown')"),
  value: z
    .string()
    .optional()
  ...
  direction: z
    ...
    .optional()
    .describe("Direction for 'scroll' action (default: 'down')"),
  amount: z
    .number()
    .int()
    .min(0)
    .optional()
    .describe("Pixels for 'scroll' action (default: 500)"),
});
  ...
  server.registerTool(
    "browser_run_steps",
      ...
      profileId: z
        ...
        .optional()
        .nullable()
        .describe("Browser profile ID (e.g. 'bpr_xxxx'). Omit to run in temporary ephemeral mode"),
      tabIndex: z
        .number()
        .int()
        .min(0)
        .optional()
        .describe("Tab index to reuse (0-based). Omit to create a new tab that auto-closes"),
      steps: z
        .array(stepSchema)
        .min(1)
        .describe("Array of browser action steps to execute in sequence"),
      continueOnError: z
        .boolean()
        .optional()
        .describe("If true, continue executing remaining steps even when one fails (default: false)"),
      includePageState: z
        .boolean()
        .optional()
        .describe("If true, append current URL + title + focused element info after each step (default: false)"),
        },
      },
    async (params: {
      ...
      try {
        const { runBatchSteps } = await import(
          "../../../../modules/browsers/browser.service.js"
        );
        const result = await runBatchSteps(params as any);
        return {
