...
/**
 * MCP run_test tool — Postman-inspired sandbox execution detail (always expanded).
 */
export function McpRunTestTool({ step }: ToolDetailProps) {
...
function ResponsePanel({ result }: { result: Record<string, unknown> }) {
  ...
  // If none of the standard fields exist, show the raw result as fallback
  const hasStandardFields = stderr || stdout || body != null;

  return (
    <div className="space-y-2">
      {!hasStandardFields && errorMsg && (
        <OutputBlock label="ERROR" variant="error">
          {rawText(errorMsg)}
        </OutputBlock>
      )}
      {!hasStandardFields && message && (
        <OutputBlock label="MESSAGE" variant="error">
          {rawText(message)}
        </OutputBlock>
      )}
      {!hasStandardFields && !errorMsg && !message && (
        <OutputBlock label="RAW" variant="default">
          {rawText(result)}
        </OutputBlock>
      )}
      {stderr && (
        <OutputBlock label="STDERR" variant="error">
          {rawText(stderr)}
        </OutputBlock>
      )}
      {body != null && (
      ...
      {stdout && (
        <OutputBlock label="STDOUT" variant="muted">
          {rawText(stdout)}
        </OutputBlock>
      )}
    </div>
